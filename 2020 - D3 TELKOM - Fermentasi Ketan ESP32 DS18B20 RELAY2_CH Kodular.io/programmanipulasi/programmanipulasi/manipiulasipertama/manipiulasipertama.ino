...
void loop() {
  ...
  {   
    ...
    if (manipulasipertama >= 4.0 || manipulasipertama == 4.0)
    {Firebase.setString("kondisi/ph", "FermentasiSelesai");}
    if (manipulasipertama <= 4.0)
...
void parsing(){
  ...
  Firebase.pushFloat("PH/data",manipulasipertama);
  Firebase.setFloat("PH/latest",manipulasipertama);
}
